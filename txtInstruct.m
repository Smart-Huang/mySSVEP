...
Screen('FrameRect',wptr,[0,0,255],wrect,3);
...
for i=pageline(1,curPage):pageline(2,curPage)
    if ~isnan(txts(i).rect)
        ...
        DrawFormattedText(wptr,txts(i).content,txts(i).x,txts(i).y);
    end
end
...
Screen('DrawText',wptr,'<--向后翻页',x1,y1,[255 0 255]);
Screen('DrawText',wptr,'-->或空格键向前翻页',x2 ,y2,[255 0 255]);
Screen('DrawText',wptr,[num2str(curPage) '/' num2str(pages)],x3,y3,[255 0 255]);
Screen('DrawText',wptr,title,x4,y4,[255 0 255]);
Screen('Flip',wptr);
...
while KbCheck; end
if ~nofile
    DrawFormattedText(wptr,'下面开始练习，按任意键继续','center','center',color);
    Screen('Flip',wptr);
    KbWait;
end
ListenChar(0);
Screen('TextColor',wptr,oldcolor);
Screen('TextSize',wptr,olds);
clear txts pageline;
ret=0;
end
